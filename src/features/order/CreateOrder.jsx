    ...
    <div className="px-4 py-6">
      ...
      <Form method="POST">
        ...
        <div className="mb-5 flex gap-2 flex-col sm:flex-row sm:items-center  relative">
          ...
          <div className="grow">
            <input className="input w-full" type="text" name="address" required defaultValue={address} />
            {addressStatus === 'error' && <p className="text-sm mt-2 text-red-700 bg-red-100 p-2 rounded-md">{errorAddress
            }</p>}
          </div>
          {!position.latitude && !position.longitude && <span className="absolute right-[5px] top-[3px] sm:z-40 md:right-[5px] md:top-[3px] z-40">
...
export async function action({ request }) {
  const formData = await request.formData(); //form data provided by browser
  ...
  // if everything is ok create newOrder and redirect
